...
/**
 * SZringet készít nullával lezáralapjait karaktersorozatból (C-sztringből)
...
String addString(const String& s0, char ch) {
    String res, temp;
    createString(temp, ch);         // char-ból sztring
    res = addString(s0, temp);      // összefűzés
    disposeString(temp);            // temp felszabadítása (memóriaszivárgás javítása!)
    return res;
}
